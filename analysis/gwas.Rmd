...
candidates <- results_df %>% group_by(SNP) %>% count() %>% filter(n >= 2)
candidates
```
...
pav_table %>% 
  filter(Individual == 'GlymaLee.02G228600.1.p') %>% 
  pivot_longer(!Individual, names_to = 'Line', values_to = 'Presence') %>% 
  inner_join(yield) %>% summarise(median_y= median(Yield))
```
...
```{r}
candidates <- candidates %>% filter(SNP != 'GlymaLee.02G228600.1.p')
```
